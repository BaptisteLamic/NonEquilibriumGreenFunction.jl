...
- **Package management**: Always use `Pkg.add()`, `Pkg.develop()`, `Pkg.instantiate()` - **NEVER manually edit Project.toml or Manifest.toml** (Manifest.toml is machine-generated).
...
- **TestItems.jl**: Uses `@testitem` blocks that are automatically discovered by `@run_package_tests` - no manual includes needed.
...
- **NEVER manually edit Manifest.toml** - it is machine-generated by Pkg. Always use `Pkg.add()`, `Pkg.develop()`, `Pkg.rm()`, etc. instead.
- **NEVER manually edit any Manifest.toml file** (including in subdirectories like `examples/`, `test/`, `benchmark/`) - these are all machine-generated.
