...
function KernelDomain(x_lims, y_lims=x_lims; n_steps::Int)
    return KernelDomain(first(x_lims), last(x_lims), first(y_lims), last(y_lims), n_steps)
end

function Base.size(domain::KernelDomain,i)
    if i in (1,2)
        return domain.n_steps
    end
    return 1
end
function Base.size(domain::KernelDomain)
    return (size(domain,i) for i in 1:2)
end
...
function Base.size(kf::KernelFunction, dim)
    if dim in (1,2)
        n = size(domain, dim) * kf.blocksize
        return n
    end
    return 1
end
function Base.size(kf::KernelFunction)
    return (size(kf, i) for i in 1:2)
end
