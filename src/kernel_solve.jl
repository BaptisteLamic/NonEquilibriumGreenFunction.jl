...
function kernel_ldiv(A::AbstractKernel,B::AbstractKernel)
    error("$(typeof(A)) \\ $(typeof(B)): not implemented")
end

function kernel_ldiv(A::TimeLocalKernel,B::AbstractKernel)
    @assert iscompatible(A,B)
    similar(B, matrix(A) \ matrix(B))
end

function kernel_ldiv(Aδ::TimeLocalKernel,Ar::RetardedKernel, B::RetardedKernel)
...
=#
function kernel_ldiv(A::RetardedKernel, B::RetardedKernel)
    @assert iscompatible(A,B)
    cp=compression(A)
    bs=blocksize(A)
