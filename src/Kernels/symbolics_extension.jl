...
function EGraphs.egraph_reconstruct_expression(::Type{BasicSymbolic{K}}, op, args; metadata=nothing, exprhead=nothing) where {K<:AbstractOperator}
    SymbolicUtils.Term{K}(op, args)
end
...
function _simplify_kernel(expr::BasicSymbolic{K}) where {K}
    theory = @theory x y z a b c m n begin
        0 * x => 0
        0 + x => x
        x + 0 => x
        0 - x => -(x)
        ...
        x * n::Number => n * x
        n::Number * x + m::Number * x => (n + m) * x
        x + x => 2 * x
        n::Number * x + x => (n + 1) * x
        n::Number * x - x => (n - 1) * x
        a::Number * b::Number * z => (a * b) * z
        n::Number * (x + y) => n * x + n * y
        inv(inv(a::BasicSymbolic{Kernel})) => a 
    end
    g = EGraph(expr)
    settermtype!(g, BasicSymbolic{K})
